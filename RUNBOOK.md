...
3. Определи, какой сигнал отклоняется:
   ...
   - `Latency p95 / p99`
   - `Requests / sec`
4. Проверь панель логов Loki (`{job="app"} | json`) в момент срабатывания.
...
- Проверь логи Loki: `{job="app", level="ERROR"} | json | event="error_endpoint"`.
- Проверь неожиданные исключения: `{job="app", level="ERROR"} | json | event="unhandled_exception"`.
...
- Если ошибка не синтетическая:
  ...
  - проверь логи: `{job="app", level="ERROR"} | json | event="unhandled_exception"`
...
- Проверь логи Loki: `{job="app", level="WARNING"} | json | event="slow_endpoint"`.
- Найди медленные запросы: `{job="app"} | json | duration_ms > 1000`.

### Действия

- Если это синтетический тест: прекрати трафик на `/slow`.
- Если проблема реальная:
  - определи медленный эндпоинт: `{job="app"} | json | duration_ms > 1000`
